...
def clust_new(result):
    all_result = [num for sublist in result for num in sublist]
    num = len(all_result)
    new_result = np.zeros(num)
    for i in range(num):
        for idx, r in enumerate(result):
            if i in r:
                new_result[i]=idx
                break
    return new_result, len(result)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", default=[
                                                # "ADClust_datasets/Baron_Human_normalized.h5ad",
    ...
    parser.add_argument("--log_dir", default="scqcut.log", help="log directory")
    ...
    for data_dir in args.data_dir:
        ...
        while trial< args.trials:
            ...
            try:
                myObj = scQcut()
                clust, net, k, clusts, dqn, range1 = myObj.getAutoAsymGeomComm(data, 'euclidean')
                
                # logging.info(myObj.getAutoAsymGeomComm(data, 'euclidean'))
                # logging.info("Trial {}".format(trial))

                new_clust, K =clust_new(clust)
                Ks.append(K)
                # logging.info("K")
                # logging.info(K)
                ...
                # logging.info("NMI")
                # logging.info(nmi(labels, new_clust))

                # logging.info("ARI")
                # logging.info(ari(labels, new_clust))
                NMI = nmi(labels, new_clust)
                ARI = ari(labels, new_clust)
                NMIs.append(NMI)
                print("NMI:",NMI)
                ARIs.append(ARI)
                print("ARI:" ,ARI)
                logging.info("Trial {} | K: {} | NMI: {:.3f} | ARI: {:.3f}".format(trial, K , NMI, ARI))
